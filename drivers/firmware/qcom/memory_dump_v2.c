	...
	for (i = 0; i < MAX_CORE_NUM; i++) {
		if (cpudata->sprdata[i]) {
			...
			cpudata->sprdata[i]->used_memory = (SPR_DATA_HEADER_SIZE +
				SPR_INPUT_DATA_TAIL_SIZE) * sizeof(u32);
			...
			p->regdump_output_byte_offset = (SPR_DATA_HEADER_SIZE +
				SPR_INPUT_DATA_TAIL_SIZE) * sizeof(u32);
			memset((u32 *)cpudata->sprdata[i]->dump_vaddr +
...
{
	int ret = 0;
	u32 system_regs_input_index, regdump_output_byte_offset, tmp;
	//overflow?
	...
	if (check_mul_overflow(core_reg_num, 2u, &tmp) ||
	    check_add_overflow(tmp, (u32)SYSTEM_REGS_INPUT_INDEX,
			       &system_regs_input_index) ||
	    check_add_overflow(system_regs_input_index, 1u, &tmp) ||
	    check_mul_overflow(tmp, (u32)sizeof(u32),
			       &regdump_output_byte_offset))
		return -EINVAL;

	mutex_lock(&cpudata->mutex);

	if (regdump_output_byte_offset >= cpudata->cpussregdata->size ||
	    regdump_output_byte_offset / sizeof(u32)
	...
	memset((u32 *)cpudata->cpussregdata->dump_vaddr + PERCORE_INDEX, 0x0,
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	p = (u32 *)cpudata->cpussregdata->dump_vaddr;
	...
	index_end = system_index_start +
			cpudata->cpussregdata->sys_reg_size / sizeof(u32) + 1;
...
static int config_cpuss_register(struct device *dev,
				 u32 *p, u32 index, char cmd,
		u32 register_offset, u32 val)
{
	int ret = 0;
...
{
	int ret;
	u32 register_offset, val, reserve_size = 4, per_core = 0;
	int nval;
	char cmd;
	u32 num_cores;
	u32 extra_memory;
	...
	if (per_core) { /* per-core register */
		...
		system_reg_end_index = *(p + SYS_REG_INPUT_INDEX) +
				cpudata->cpussregdata->sys_reg_size / sizeof(u32);

		if (cmd == 'r' && reserve_size == 4)
			extra_memory = sizeof(u32) + reserve_size;
		else
			extra_memory = sizeof(u32) * 2 + reserve_size;
		...
		if (cmd == 'r' && val == 4) {
			...
			cpudata->cpussregdata->sys_reg_size += sizeof(u32) * 2;
		}
			...
			const char *buf, size_t size)
{
	...
	update_reg_dump_table(dev, CORE_REG_NUM_DEFAULT);//???????ignore ret
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < MAX_CORE_NUM; i++) {
		...
		p = (u32 *)cpudata->sprdata[i]->dump_vaddr;
			...
			const char *buf, size_t size)
{
	...
	reserved = (SPR_INPUT_DATA_SIZE + SPR_OUTPUT_DATA_SIZE) * sizeof(u32);

	mutex_lock(&cpudata->mutex);
	if (cpudata->sprdata[cpu_num]) {
		p = (u32 *)cpudata->sprdata[cpu_num]->dump_vaddr;
		...
		if (cpudata->sprdata[cpu_num]->size >
			...
			*(p + OUTPUT_DUMP_INDEX) = (SPR_DATA_HEADER_SIZE +
				index + SPR_INPUT_DATA_TAIL_SIZE + 1) * sizeof(u32);
			...
			const char *buf, size_t size)
{
	...
	reset_sprs_dump_table(dev);//?????ignore ret
...
u32 msm_dump_table_version(void)
{
...
static int init_memory_dump(void *dump_vaddr, phys_addr_t phys_addr)
{
	...
	entry.id = MSM_DUMP_TABLE_APPS;//? should we use parameter rather than fixed value?
			   ...
			   void *dump_vaddr, u32 size, u32 id)
{
	...
	static int initialized; //? should we maintain this value in cpudata structure
...
static int mem_dump_alloc(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(node, child_node) {
		...
		if (id == CPUSS_REGDUMP ||
		    (id >= SPR_DUMP_CPU0 && id <= SPR_DUMP_CPU7))//？？？？？？use else
